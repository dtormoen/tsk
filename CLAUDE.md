...
- Tasks create branches: `tsk/{task-type}/{task-name}/{task-id}` (e.g., `tsk/feat/add-user-auth/a1b2c3d4`). The task's `branch_name` is always this generated name. When `target_branch` is set, `tsk` attempts a fast-forward push to that branch at fetch time, falling back to the generated branch on failure.

...
- **Git Operations** (`src/git.rs`, `src/git_sync.rs`, `src/git_operations.rs`, `src/repo_utils.rs`): Repository cloning with working directory overlay, branch creation, result fetching. Uses git CLI (not libgit2) for commits to support clean/smudge filters. `GitSyncManager` for concurrent access safety. Supports submodules, git worktrees, and Git LFS.

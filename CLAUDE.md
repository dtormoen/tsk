...
- `run::run_tui()`: Async event loop multiplexing crossterm events, server events, and periodic timers; accepts a `docker_client` for TUI-driven cancel operations; `sort_tasks_for_display()` applies display ordering (non-terminal tasks first, children grouped after parents, newest-first tiebreaker)
...
- Infrastructure logging: `TaskRunner::run_in_container` creates `agent.log` early (before Docker image build) and writes infrastructure `LogLine` entries (image build, agent launch, saving changes, branch result) as JSON-lines. `DockerManager::run_container_inner` opens the same file in append mode so agent output follows infrastructure messages. The `write_log` helper writes to the file and calls `emit_or_print` so messages appear in both the log panel and global header. `fetch_changes` returns a `FetchResult` with `has_changes` and non-fatal `warnings` (e.g. submodule fetch failures, git-town errors) that are logged via `write_log` rather than printed to stderr

...
**Storage** (`src/context/`)
- `TskEnv`: Manages directory paths (data_dir, runtime_dir, config_dir) and runtime environment settings (editor, terminal type); also provides `open_agent_log(task_id)` to open a task's `agent.log` in append mode. `TaskRunner` wraps the returned file in a `TaskLogger` for infrastructure messages; `DockerManager::run_container_inner` opens the same file in append mode to stream raw agent output. `tsk`-specific env vars (`TSK_DATA_HOME`, `TSK_RUNTIME_DIR`, `TSK_CONFIG_HOME`) take precedence over XDG vars, enabling isolated testing without affecting other XDG-aware software
...
- `input::handle_event()`: Keyboard (vim-style + arrows) and mouse input handling; returns `Option<TuiAction>` for async operations (cancel/delete) executed by the event loop
- `run::run_tui()`: Async event loop multiplexing crossterm events, server events, and periodic timers; accepts a `docker_client` for TUI-driven cancel operations; `sort_tasks_for_display()` applies display ordering (non-terminal tasks first, newest-first within each group)
- Stdout suppression: `DockerManager` derives a `suppress_stdout` flag from `event_sender.is_some()` to suppress raw agent log output in TUI mode
- Two output paths: Task-scoped messages use `TaskLogger` (`src/agent/task_logger.rs`), which writes to the task's `agent.log` and prints to stdout/stderr when `!suppress_stdout`; it never emits `ServerEvent`. Global messages (task scheduled/completed/failed, server lifecycle) use `emit_or_print` which sends `ServerEvent` in TUI mode or prints to stdout/stderr otherwise
- Infrastructure logging: `TaskRunner::run_in_container` creates `agent.log` early (before Docker image build) and wraps it in a `TaskLogger`. All infrastructure `LogLine` entries (image build, agent launch, saving changes, branch result) are written as JSON-lines via the logger. `DockerManager::run_container_inner` opens the same file in append mode so agent output follows infrastructure messages. `DockerImageManager`, `ProxyManager`, and `DockerBuildLockManager` accept `&TaskLogger` for task-scoped messages. `fetch_changes` returns a `FetchResult` with `has_changes` and non-fatal `warnings` (e.g. submodule fetch failures, git-town errors) that are logged via `TaskLogger` rather than printed to stderr
